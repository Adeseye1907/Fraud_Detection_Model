...
if st.button("Predict Fraud"):
    ...
    # Create input dictionary with reasonable defaults
    input_dict = {col: 0 for col in FEATURE_NAMES}
    
    # Fill the features you are collecting from user
    input_dict.update({
        'amount': amount,
        'hour': hour,
        ...
        # Add more fields here as you collect them (channel, merchant_category, etc.)
    })
    ...
    for col, le in encoders.items():
        if col in input_df.columns:
            ...
            # Handle unseen labels safely by mapping unknown values to a known one
            # (most common safe approach for production apps)
            known_classes = set(le.classes_)
            input_df[col] = input_df[col].apply(
                lambda x: x if x in known_classes else le.classes_[0]   # fallback to the first seen class
            )
            
            input_df[col] = le.transform(input_df[col])

    # Handle id_encoders (transaction_id, customer_id) the same way
