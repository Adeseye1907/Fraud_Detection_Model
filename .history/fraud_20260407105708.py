...
with col2:
    ...
    channel = st.selectbox("Channel", ["Online", "POS", "ATM", "Mobile App", "USSD", "Others"])

with col3:
    tx_count_24h = st.number_input("Tx Count 24h", value=5)
...
tx_count_total = st.number_input("Total Tx Count", value=50)
...
merchant_category = st.selectbox("Merchant Category", ["Retail", "Food & Beverage", "Transport", "Electronics", "Fashion", "Others"])
bank = st.selectbox("Bank", ["GTBank", "Access Bank", "Zenith Bank", "UBA", "FirstBank", "Others"])
location = st.selectbox("Location", ["Lagos", "Abuja", "Port Harcourt", "Ibadan", "Kano", "Others"])
...
if st.button("Predict Fraud", type="primary"):
    ...
    # Create input with exact same features and order as during training
    input_data = {col: 0 for col in FEATURE_NAMES}
    
    # Fill all collected values
    input_data.update({
        'amount': amount,
        'hour': hour,
    ...
    # Safe Encoding for all categorical columns
    ...
    # Safe Encoding for ID columns (if any)
    ...
    # Scaling - Column by column (because your scaler was fitted that way)
    input_scaled = input_df.copy()

    for col in input_df.columns:
        try:
            scaled_values = scaler.transform(input_df[[col]])
            input_scaled[col] = scaled_values.ravel()
        except:
            pass  # skip if scaling fails on this column

    # Final input for model
    final_input = input_scaled.values.astype('float64')
    ...
    if prediction[0] == 1:
        st.error(f"⚠️ Fraud Detected ({probability*100:.2f}% probability)")
    else:
        st.success(f"✅ Legitimate Transaction ({probability*100:.2f}% probability)")
