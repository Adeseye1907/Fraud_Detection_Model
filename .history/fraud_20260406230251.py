...
if st.button("Predict Fraud"):
    ...
    # Create input with ALL features in exact order as during training
    ...
    # Safe Encoding for categorical + ID columns
    ...
    # ====================== FIXED SCALING ======================
    # Since your scaler was fitted column-by-column (only on high std columns),
    # we must scale column by column the same way
    input_scaled = input_df.copy()

    for col in input_df.columns:
        # Only scale columns that were actually scaled during training
        # (i.e., those with high std originally)
        if input_df[col].std() > 1000 or True:   # For now we try scaling all numeric
            try:
                # Reshape to 2D for scaler
                input_scaled[col] = scaler.transform(input_df[[col]])
            except:
                pass  # if scaler fails on this column, leave as is

    # Convert to numpy for prediction
    final_input = input_scaled.values.astype(float)
