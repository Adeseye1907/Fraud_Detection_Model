...
st.set_page_config(page_title="NIBSS Fraud Guard", page_icon="🛡️", layout="wide")
warnings.filterwarnings('ignore')

# 2. Data Loading with Caching
@st.cache_data
def load_data():
    return pd.read_csv('nibss_fraud_dataset.csv')
...
st.markdown("<h4 style = 'margin:30px; color:#FFC470; text-align:center; font-family:Serif'>Built by Adeseye </h4>", unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html = True)

# 4. Hero Image & Background
try:
    st.image('fraud detection.png', use_container_width=True)
except:
    st.warning("Banner image 'fraud detection.png' not found.")

st.divider()

st.markdown("<h2 style = 'color: #F7C566; text-align: center; font-family: montserrat '>Background Of Study</h2>", unsafe_allow_html = True)
st.markdown("""
Fraud detection is a critical component of **Data-Driven Decision Making Systems (DDDM)** in digital banking. 
In modern fintech environments such as **Moniepoint**, high transaction volumes create opportunities for both 
legitimate activity and fraudulent behavior. This study leverages machine learning to distinguish between 
the two, focusing on recall and ROC-AUC to minimize financial loss.
""")

st.divider()

# 5. Data Preview & Basic Insight
st.subheader("📊 Dataset Overview")
st.dataframe(ds.head(50), use_container_width=True) # Showing first 50 rows for performance

# Quick Insight: Fraud vs Legitimate
fraud_counts = ds['is_fraud'].value_counts().reset_index()
fraud_counts.columns = ['Status', 'Count']
fraud_counts['Status'] = fraud_counts['Status'].map({0: 'Legitimate', 1: 'Fraud'})

fig = px.pie(fraud_counts, values='Count', names='Status', 
             title="Distribution of Fraud in Dataset",
             color_discrete_sequence=['#FFC470', '#DD5746'])
