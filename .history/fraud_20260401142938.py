...
st.markdown("<h1 style = 'color: #DD5746; text-align: center; font-size:60px; font-family: Monospace'>FRAUD PREDICTION </h1>", unsafe_allow_html = True)
...
st.markdown('Fraud detection is a critical component of Data-Driven Decision Making Systems (DDDM) in digital banking, where transaction integrity and customer trust directly impact financial stability and platform credibility. In modern fintech environments such as Moniepoint, a high volume of digital transactions is processed in real time, creating opportunities for both legitimate activity and fraudulent behavior. Fraudulent transactions are typically rare but high-impact events, making them difficult to detect using traditional rule-based systems alone. Several transactional and behavioral factors influence fraud risk, including transaction amount, frequency, time of activity, device characteristics, geographic location, and customer behavior patterns. These variables provide insight into anomalies that may indicate suspicious activity. This study aims to analyze transaction-level data to identify patterns associated with fraudulent behavior and to develop predictive models capable of distinguishing between legitimate and fraudulent transactions. By leveraging machine learning techniques within a DDDM framework, the analysis incorporates empirical evidence from model performance metrics such as recall, precision, ROC-AUC, and F1-score to guide decision-making. The findings support the development of an effective fraud detection system that enhances real-time monitoring, reduces financial losses, and strengthens risk management strategies. Ultimately, the insights generated enable digital banking platforms like Moniepoint to proactively detect fraud, improve operational security, and maintain customer trust while supporting scalable and data-driven financial services.')
...
def main():
    st.title('Fraud detection Prediction App with Innovation')
    st.markdown("""
        Predict the likelihood of a r innovative **Coefficient** and **CCRI** features.
    """)
