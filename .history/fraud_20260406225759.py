...
if st.button("Predict Fraud"):
    ...
    # Create input dictionary - start with ALL expected features from training
    input_dict = {col: 0 for col in FEATURE_NAMES}   # default 0 for missing features
    
    # Fill in the values you collected from the user
    input_dict.update({
        'amount': amount,
        'hour': hour,
        ...
        # Add other fields you collect (e.g. channel, merchant_category, etc.) here
    })

    # Create DataFrame with features in EXACT order the scaler expects
    input_df = pd.DataFrame([input_dict])[FEATURE_NAMES]   # ← This line fixes the column order

    # === Safe Encoding for Categorical Columns (handles unseen labels) ===
    for col, le in encoders.items():
        if col in input_df.columns:
            input_df[col] = input_df[col].astype(str)
            # Fallback for any unseen value
    ...
    # === Safe Encoding for ID columns (transaction_id, customer_id) ===
    ...
    # === Scale the data - now columns are in correct order ===
    input_scaled = scaler.transform(input_df)          # returns numpy array
