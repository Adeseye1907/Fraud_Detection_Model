...
if st.button("Predict Fraud"):
    ...
    # Create input with ALL features in EXACT same order as during training
    input_data = {col: 0 for col in FEATURE_NAMES}   # default value
    
    # Fill the values you are getting from user input
    input_data.update({
        'amount': amount,
        'hour': hour,
        ...
        # You can add more here later (channel, merchant_category, etc.)
    })

    # Create DataFrame with EXACT column order as during fit
    ...
    # ====================== FINAL FIX ======================
    # Convert to numpy array BEFORE scaling to bypass feature name check
    input_array = input_df.values.astype(float)          # Important: .values
